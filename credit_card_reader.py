...
class AdvancedImageProcessor:
    ...
    def preprocess(self, image: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            cv2.imwrite(os.path.join(debug_dir, "original_gray.jpg"), gray)
            ...
            cv2.imwrite(os.path.join(debug_dir, "method1_bilateral_clahe.jpg"), enhanced1)
            ...
            cv2.imwrite(os.path.join(debug_dir, "method2_gaussian_adaptive.jpg"), enhanced2)
            
            # Method 3: Otsu's thresholding
            _, enhanced3 = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            processed_images.append(enhanced3)
            cv2.imwrite(os.path.join(debug_dir, "method3_otsu.jpg"), enhanced3)
            
            # Method 4: Denoise + CLAHE
            denoised = cv2.fastNlMeansDenoising(gray)
            enhanced4 = self.clahe.apply(denoised)
            processed_images.append(enhanced4)
            cv2.imwrite(os.path.join(debug_dir, "method4_denoise_clahe.jpg"), enhanced4)
            
            # Method 5: Sharpen + CLAHE
            kernel = np.array([[-1,-1,-1], [-1,9,-1], [-1,-1,-1]])
            sharpened = cv2.filter2D(gray, -1, kernel)
            enhanced5 = self.clahe.apply(sharpened)
            processed_images.append(enhanced5)
            cv2.imwrite(os.path.join(debug_dir, "method5_sharpen_clahe.jpg"), enhanced5)
            ...
            # Save final result
            cv2.imwrite(os.path.join(debug_dir, "final_preprocessed.jpg"), final)
    ...
    def extract_digits(self, image: np.ndarray) -> List[Tuple[int, np.ndarray]]:
        ...
        try:
            ...
            cv2.imwrite(os.path.join(debug_dir, "preprocessed.jpg"), gray)
            ...
            for method_idx, thresh_method in enumerate(thresh_methods):
                try:
                    # Apply thresholding
                    thresh = thresh_method(gray)
                    
                    # Save thresholded image
                    cv2.imwrite(os.path.join(debug_dir, f"threshold_{method_idx}.jpg"), thresh)
                    ...
                    # Save after morphological operations
                    cv2.imwrite(os.path.join(debug_dir, f"morph_{method_idx}.jpg"), thresh)
                    ...
                    for contour in contours:
                        ...
                        if (0.2 < aspect_ratio < 1.0 and  # Slightly wider range for aspect ratio
                            area > 50 and  # Back to original area threshold
                            y > middle_third_y * 0.7 and  # Slightly wider vertical range
                            y < middle_third_y * 2.3):
                            
                            # Check if this contour is too close to any existing contour
                            too_close = False
                            for ex_x, _ in valid_contours:
                                if abs(x - ex_x) < 8:  # Slightly increased minimum distance
                                    too_close = True
                                    break
                            ...
                            if not too_close:
                                ...
                                # Save the extracted digit for debugging
                                digit_img = gray[y:y+h, x:x+w]
                                cv2.imwrite(os.path.join(debug_dir, f"digit_{len(valid_contours)}.jpg"), digit_img)
    ...
    try:
        ...
        for processed in processed_images:
            ...
            for idx, (_, contour) in enumerate(digit_contours[:16]):
                ...
                # Convert to tensor
                digit_tensor = torch.FloatTensor(digit).unsqueeze(0).unsqueeze(0) / 255.0
                ...
                with torch.no_grad():
                    ...
                    # Debug print: show raw probabilities and prediction
                    print(f"Digit {idx+1}: Probabilities = {probabilities.cpu().numpy().flatten()}, Predicted = {predicted_digit.item()}, Confidence = {confidence.item():.4f}")
...
def main():
    ...
    try:
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = AdvancedCreditCardNet(config)
        
        # Load checkpoint and extract model state dict
        checkpoint = torch.load('best_credit_card_model.pth', map_location=device)
...
def four_point_transform(image: np.ndarray, pts: np.ndarray) -> np.ndarray:
    """Apply perspective transform to get top-down view of the card"""
    try:
...
def preprocess_digit(digit: np.ndarray) -> Optional[np.ndarray]:
    """Preprocess a single digit for recognition"""
    try:
        # Ensure the digit is grayscale
        if len(digit.shape) == 3:
            digit = cv2.cvtColor(digit, cv2.COLOR_BGR2GRAY)
        ...
        logger.error(f"Error in preprocess_digit: {str(e)}")
        return None
